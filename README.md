<h1 align="center">Hi 👋, I'm Priyadharshnee</h1>
<h3 align="center">Aspiring Data Engineer | B.Tech IT @ SRM | Fast Learner | Future SDE 🔥</h3>

---

### 👩‍💻 About Me
- 🎓 Final Year B.Tech IT Student at SRM IST  
- 🎯 Goal: Data Engineer or Software Developer @ Product-Based Company  
- 💼 Currently learning Python, SQL, Git, and Building Projects  
- 💡 Passionate about AI, Big Data, and Cloud Tools  
- 💬 Ask me about anything tech — I’m always learning  
- ⚡ Fun Fact: I love late-night coding and AI summits 🧠✨  

---

### 📚 Learning Roadmap (July–Aug 2025)
- [x] Python Mastery
- [x] SQL Practice (LeetCode)
- [x] Git + GitHub Projects
- [x] Data Engineering Project
- [x] Java & DSA for SDE Roles
- [x] Personal Portfolio Websites (SDE & Data Engineer)

---

### 🛠️ Languages & Tools
![Python](https://img.shields.io/badge/-Python-black?style=flat&logo=Python)
![SQL](https://img.shields.io/badge/-SQL-black?style=flat&logo=mysql)
![Java](https://img.shields.io/badge/-Java-black?style=flat&logo=java)
![Git](https://img.shields.io/badge/-Git-black?style=flat&logo=git)
![VS Code](https://img.shields.io/badge/-VSCode-black?style=flat&logo=visualstudiocode)
![GitHub](https://img.shields.io/badge/-GitHub-black?style=flat&logo=github)

---

### 🌟 Featured Projects
📌 `Netflix Data Analysis` – Analyzing global Netflix content using Python & Pandas  
📌 `ETL Pipeline Demo` – Simulating a data pipeline using Python, SQL & Cron  
📌 `Student CRUD App` – Java/Flask app for managing student records (SDE Portfolio)

> 💡 More coming soon...

---

### 🌐 Portfolio Website
> 🔗 [Coming soon... Stay tuned!]

---

### 📈 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Priyadharshnee&show_icons=true&theme=radical" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Priyadharshnee&theme=radical" />
</p>

---

### ⌛ Weekly Coding Activity & Practice Platforms

- 🧠 Practicing DSA & SQL on:
  - [LeetCode 🧩](https://leetcode.com/u/priyadharshneeraghu/)
  - [HackerRank 💻](https://www.hackerrank.com/profile/priyadharshneer1)


> 🚀 Let’s connect and grow together on [LinkedIn](https://www.linkedin.com/in/priyadharshnee-r-4ba334256/)
